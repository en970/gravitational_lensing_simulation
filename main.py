...
def add_galaxy(background, cx, cy, radius, max_brightness):
    ...
    background[y_min:y_max, x_min:x_max] = np.maximum(background[y_min:y_max, x_min:x_max], brightness)
...
def add_nebula(background, cx, cy, radius):
    y_min = max(0, cy - radius); y_max = min(BG_HEIGHT, cy + radius)
    x_min = max(0, cx - radius); x_max = min(BG_WIDTH, cx + radius)
    if y_max <= y_min or x_max <= x_min: return
    y, x = np.ogrid[y_min:y_max, x_min:x_max]
...
def add_galaxy_cluster(background, cx, cy, radius):
    ...
    for i in range(num):
        ...
        gr = np.random.randint(8, 20); gb = np.random.randint(60, 120)
...
def render_lensed_view(lens_x, lens_y, mass):
    #calculates light bending
    y_img, x_img = np.mgrid[0:HEIGHT, 0:WIDTH].astype(np.float32)
    dx = x_img - lens_x; dy = y_img - lens_y
    theta = np.sqrt(dx**2 + dy**2); theta = np.maximum(theta, 0.1)
    theta_E = mass * 20.0; r_schwarzchild = mass * 3.0
    deflection = (theta_E**2) / theta
    beta_mag = theta - deflection
    ux = dx / theta; uy = dy / theta
    src_x = lens_x + beta_mag * ux + PADDING
    src_y = lens_y + beta_mag * uy + PADDING
    src_xi = np.clip(src_x.astype(np.int32), 0, BG_WIDTH - 1)
    src_yi = np.clip(src_y.astype(np.int32), 0, BG_HEIGHT - 1)
    output = background_texture[src_yi, src_xi].copy().astype(np.float32)
    
    with np.errstate(divide='ignore', invalid='ignore'):
        magnif = np.abs(theta / (beta_mag + 0.1))
        magnif = np.clip(magnif, 0.5, 4.0)
    output = output * magnif
    
    #black hole center
    ev_horizon = theta < r_schwarzchild
    photon_sphere = (theta >= r_schwarzchild) & (theta < r_schwarzchild * 1.5)
    output[ev_horizon] = 0
    ...
    return pygame.surfarray.make_surface(rgb.swapaxes(0, 1)), r_schwarzchild


def draw_ui(mass):
    font = pygame.font.Font(None, 22)
    texts = [("Mouse: Move | Scroll: Mass", (120, 120, 120)), ("Mass: " + str(round(mass, 1)), (200, 200, 200))]
    y = 8
    ...
    while running:
        for event in pygame.event.get():
            if event.type == pygame.QUIT: running = False
                ...
                if event.y > 0: lens_mass = min(lens_mass * 1.1, 25.0)
                else: lens_mass = max(lens_mass / 1.1, 1.0)
        
        lens_x, lens_y = pygame.mouse.get_pos()
        keys = pygame.key.get_pressed()
        if keys[pygame.K_UP]: lens_mass = min(lens_mass * 1.02, 25.0)
        if keys[pygame.K_DOWN]: lens_mass = max(lens_mass / 1.02, 1.0)
        
        lensed_surf, r_s = render_lensed_view(lens_x, lens_y, lens_mass)
        screen.blit(lensed_surf, (0, 0))
